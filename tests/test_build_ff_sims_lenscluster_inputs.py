...
def test_render_converts_catalogs_and_writes_loadable_pars(tmp_path: Path) -> None:
    ...
    assert len(active_ares_member_lines) == 1
    assert active_ares_member_lines[0].split()[:4] == ["6", "11.00000000", "11.00000000", "1.00000000"]
    assert active_ares_member_lines[0].split()[6] == "21.000000"
    ...
    assert "potentiel S1" in ares_par_text
    assert "profil 14" in ares_par_text
    assert "potentiel G5" in ares_par_text
    ares_g4_block = ares_par_text.split("potentiel G4", maxsplit=1)[1].split("potentiel G5", maxsplit=1)[0]
    ares_g5_block = ares_par_text.split("potentiel G5", maxsplit=1)[1].split("potfile", maxsplit=1)[0]
    ares_g4_vdisp = next(line.strip() for line in ares_g4_block.splitlines() if line.strip().startswith("v_disp 1"))
    ares_g5_vdisp = next(line.strip() for line in ares_g5_block.splitlines() if line.strip().startswith("v_disp 1"))
    assert float(ares_g4_vdisp.split()[3]) / float(ares_g4_vdisp.split()[2]) == pytest.approx(3.0)
    assert float(ares_g5_vdisp.split()[3]) / float(ares_g5_vdisp.split()[2]) == pytest.approx(5.0)
